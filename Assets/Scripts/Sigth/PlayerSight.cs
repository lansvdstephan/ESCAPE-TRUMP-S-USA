    ...
    void findPlayer()
    {
        ...
        if (distance < sightRadius)
        {
            ...
            if (angle < sightAngle / 2 && angle > (sightAngle * -1) / 2)
            {
                ...
                if (Physics.Raycast(new Vector3(transform.position.x, transform.position.y + 1f, transform.position.z), AngleD.normalized, out hit, distance))//send out a Raycast in the direction of the player
                {
